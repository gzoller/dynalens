...
// ---------------------- Trim ----------------------
object CTrimFn extends CompileFn:
  val name = "trim"
  val minArgs = 0
...
// ---------------------- ToLower ----------------------
object CToLowerFn extends CompileFn:
  val name = "toLower"
  val minArgs = 0
  override val maxArgs = 0
  ...
  def build(recv: Receiver, args: List[Fn[Any]])(using ctx: ExprContext) =
    ...
    else Right(ToLowerFn(recv.fn, ctx.posStr).asInstanceOf[Fn[Any]])

  override def validate(fn: Fn[?])(using ctx: ExprContext): Either[DLCompileError, Unit] = Right(())


// ---------------------- ToUpper ----------------------
object CToUpperFn extends CompileFn:
  val name = "toUpper"
  val minArgs = 0
  override val maxArgs = 0
...
// ---------------------- Interpolate ----------------------
object CInterpolateFn extends CompileFn:
  val name = "template"
  val minArgs = 0
  override val maxArgs = 0
  ...
  def build(recv: Receiver, args: List[Fn[Any]])(using ctx: ExprContext) =
    if args.nonEmpty then Left(DLCompileError(ctx.posStr, "template() takes no arguments"))
      ...
      Right(InterpolateFn(recv.fn, varMap, ctx.posStr).asInstanceOf[Fn[Any]])

  override def validate(fn: Fn[?])(using ctx: ExprContext): Either[DLCompileError, Unit] = Right(())


// ---------------------- Substring ----------------------
object CSubstringFn extends CompileFn:
  val name = "substring"
  val minArgs = 2
  ...
  def build(recv: Receiver, args: List[Fn[Any]])(using ctx: ExprContext) =
    ...
    yield SubstringFn(recv.fn, start, Some(end), ctx.posStr).asInstanceOf[Fn[Any]]

  override def validate(fn: Fn[?])(using ctx: ExprContext) =
    fn match
      case s: SubstringFn =>
        Validation.requireNumeric2(List(s.start, s.end.get), "substring", ctx.posStr)
      case _ => Right(())


// ---------------------- Replace ----------------------
object CReplaceFn extends CompileFn:
  val name = "replace"
  val minArgs = 2
  override val maxArgs = 2
  ...
  def build(recv: Receiver, args: List[Fn[Any]])(using ctx: ExprContext) =
    ...
    yield ReplaceFn(recv.fn, target, repl, ctx.posStr).asInstanceOf[Fn[Any]]
...
// ---------------------- Concat ----------------------
object CConcatFn extends CompileFn:
  val name = "+"
  val minArgs = 1
